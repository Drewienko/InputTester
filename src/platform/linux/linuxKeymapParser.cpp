...
namespace linuxKeymapParser
{
    ...
    bool parseLinuxKeyMap(const QByteArray &data, linuxKeyMap *outMap, std::vector<QString> *errors)
    {
        ...
        {
            appendError(errors, "qtKeyToVirtualKey",
                        QString("missing array (got %1)").arg(valueTypeName(qtKeyArrayValue)));
            ok = false;
        }
        else
        {
        ...
        {
            appendError(errors, "linuxScanToWinScan",
                        QString("missing array (got %1)").arg(valueTypeName(scanArrayValue)));
            ok = false;
        }
        else
        {
